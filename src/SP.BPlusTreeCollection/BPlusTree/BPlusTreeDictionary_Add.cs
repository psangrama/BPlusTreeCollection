...
namespace SP.BPlusTreeCollection.BPlusTree
{
	public partial class BPlusTreeDictionary<TKey, TValue>
	{
		...
		{
			this.root = new Leaf<TKey, TValue>(_allLeafNodes);
		...
		{
			var left = node;
			var right = node.Split(_allLeafNodes);
		...
		void AddInternal(ref Internal<TKey, TValue> parent, Internal<TKey, TValue> node, TKey key, TValue value)
		{
			if (node.IsFull)
			{
				...
				parent.Add(node.Split(_allLeafNodes), comparer);
				node = parent;
			}
			...
			else
			{
				...
				if (r.Add(key, value, comparer)) return;
				node.Add(r.Split(_allLeafNodes), comparer);
